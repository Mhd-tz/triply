...
interface TripStore {
    ...
    setPlannerDestinations: (v: { id: string; name: string; date: Date | null }[]) => void;
    setPlannerBudget: (v: string) => void;
    setPlannerCurrency: (v: string) => void;
    setPlannerVibe: (v: string[]) => void;
...
export const useTripStore = create<TripStore>((set) => ({
    ...
    setPlannerVibe: (v) => set({ plannerVibe: v }),
    setPlannerNotes: (v) => set({ plannerNotes: v }),
    setPlannerFlights: (v) => set({ plannerFlights: v }),
    addPlannerFlight: (f) => set((s) => ({ plannerFlights: [...s.plannerFlights.filter(x => x.id !== f.id), f] })),
    removePlannerFlight: (id) => set((s) => ({ plannerFlights: s.plannerFlights.filter(x => x.id !== id) })),
    clearAll: () => set({ 
        linkedTransport: null, 
        linkedStay: null,
        plannerOrigin: "",

...
import {
  X, Search, MapPin, Clock, ArrowRight, ExternalLink,
  Wand2, Sparkles, ChevronDown, Plus, Utensils,
  Compass, Landmark, Loader2, Car, Train, PersonStanding, AlertTriangle,
} from "lucide-react";
...
const CATEGORY_META: {
  ...
  color: string;
    ...
    { key: "meal", label: "Meal", icon: <Utensils className="w-3.5 h-3.5" />, color: CATEGORY_COLORS.meal },
    { key: "activity", label: "Activity", icon: <Compass className="w-3.5 h-3.5" />, color: CATEGORY_COLORS.activity },
    { key: "location", label: "Location", icon: <Landmark className="w-3.5 h-3.5" />, color: CATEGORY_COLORS.location },
  ];
...
interface DayPlan { day: number; date: string; events: any[]; }
    ...
    <div className="relative">
      ...
      <button
        ...
        className={cn(
          ...
          open ? "bg-white shadow-[0_0_0_3px_rgba(59,130,246,0.12)]" : "bg-gray-50 border-gray-200 hover:border-gray-300"
        )}
        style={open ? { borderColor: accentColor } : undefined}
      >
        <Clock className="w-3.5 h-3.5 shrink-0" style={{ color: accentColor }} />
        <span className="flex-1 font-bold text-gray-900 text-[13px] text-left">{display}</span>
            ...
            >
              ...
              <div className="flex p-0.5 bg-gray-100 rounded-lg mb-2.5">
                <button
                  ...
                  className={cn(
                    ...
                    !isPM ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"
                  )}
                >AM</button>
                <button
              ...
              <div className="flex gap-2.5">
                <div className="flex-1">
                  <p className="text-[9px] font-bold text-gray-400 uppercase text-center mb-1.5 tracking-widest">Hour</p>
                ...
                <div className="flex-1">
                  <p className="text-[9px] font-bold text-gray-400 uppercase text-center mb-1.5 tracking-widest">Min</p>
                  <div className="grid grid-cols-4 gap-1">
                    {minutes.map((min) => (
                      <button
                        key={min}
...
function PlaceAutocomplete({
  ...
  return (
    <div className="relative">
      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Search Place</label>
      <div
      ...
      >
        ...
        <input
          ...
          onFocus={() => { if (results.length > 0 && !justSelectedRef.current) setShowDropdown(true); }}
        ...
        {value && !loading && (
          <button onClick={() => { onChange(""); setResults([]); setShowDropdown(false); }} className="p-0.5">
      ...
      <AnimatePresence>
        {showDropdown && results.length > 0 && (
          <>
            ...
            >
              {results.map((place) => {
                const catMeta = CATEGORY_META.find((c) => c.key === place.category);
                return (
                  <button key={place.placeId || place.name + place.lat} onClick={() => handleSelect(place)}
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5"
                      style={{ backgroundColor: (catMeta?.color || "#94a3b8") + "15", color: catMeta?.color || "#94a3b8" }}
                    ...
                    {catMeta && (
                      <span className="text-[9px] font-bold uppercase px-2 py-0.5 rounded-md shrink-0 mt-1"
                        style={{ backgroundColor: catMeta.color + "15", color: catMeta.color }}>{catMeta.label}</span>
                    )}
                  </button>
                );
              })}
...
function PopularPlacesSection({
  destinations, onSelect, tripData, category,
}: {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-8 gap-2">
      ...
      <p className="text-xs text-gray-500 font-medium">Finding popular places nearby…</p>
    </div>
  );
        ...
        {places.slice(0, 5).map((place) => {
          ...
          return (
            ...
            >
              ...
              {/* Title + Address */}
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-bold text-gray-900 truncate leading-tight">{displayName}</p>
                {displayName !== place.name && (
                  <p className="text-[10px] text-gray-400 truncate leading-tight">{place.name}</p>
                )}
                {place.address && (
                  <p className="text-[10px] text-gray-400 truncate leading-tight mt-0.5">{place.address.split(",").slice(0, 2).join(",")}</p>
                )}
              </div>
              ...
              >
                ...
                <span className="text-[8px] font-bold uppercase tracking-tight" style={{ color: catMeta.color }}>{catMeta.label}</span>
              </div>
...
export default function AddEventModal({
  ...
  const [rating] = React.useState<number | undefined>(event?.rating || prefill?.rating);
  const [reviewCount] = React.useState<number | undefined>(event?.reviewCount || prefill?.reviewCount);
  ...
  const [transportTo, setTransportTo] = React.useState<"walk" | "transit" | "drive">("walk");
  ...
  React.useEffect(() => {
    if (config.isOpen && !prevOpenRef.current && config.mode === "add" && !prefill) {
      setTitle(""); setTime("12:00"); setEndTime("13:00");
  ...
  const activeCategory = CATEGORY_META.find((c) => c.key === type) || CATEGORY_META[1];
  ...
  const [biasCoords, setBiasCoords] = React.useState<{ lat: number; lng: number } | null>(null);

  React.useEffect(() => {
    if (activeDests.length > 0 && !biasCoords) {
      fetch(`/api/places/autocomplete?q=${encodeURIComponent(activeDests[0].name)}`)
        .then((r) => r.json())
        .then((data) => {
          if (data.results?.[0]) {
            setBiasCoords({ lat: data.results[0].lat, lng: data.results[0].lng });
          }
        })
        .catch(() => { });
    }
  ...
  // ── Fetch image for selected place ──────────────────────
  React.useEffect(() => {
    if (title && title.length > 2 && images.length === 0) {
      const cityContext = destinations[0]?.name?.split(",")[0] || "";
      fetch(`/api/places/image?name=${encodeURIComponent(title + " " + cityContext)}`)
        .then((r) => r.json())
        .then((data) => {
          if (data.imageUrl) {
            setPlaceImage(data.imageUrl);
  ...
  const handlePlaceSelect = (place: AutocompletePlace | PopularPlace) => {
    ...
    const displayName = ("translatedName" in place && place.translatedName) ? place.translatedName : place.name;
      ...
      onSave(
        {
          ...
          time: "18:00", endTime: "20:00",
          type: "location", color: CATEGORY_COLORS.location,
          lat: 35.6605, lng: 139.7291,
  ...
  const handleSave = () => {
    ...
    const selectedCat = CATEGORY_COLORS[type as keyof typeof CATEGORY_COLORS] || "#94a3b8";
    onSave(
      {
        ...event,
        ...
        rating, reviewCount, reviews, url,
      },
      targetDay
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4 py-8" onClick={onClose}>
      ...
      >
        {/* Header */}
        <div className="px-5 py-4 flex justify-between items-center shrink-0 transition-colors duration-300"
        ...
        {/* Place image banner */}
        {(images.length > 0 || placeImage) && step === "manual" && (
              ...
              <div className="flex-1 px-5 py-4 space-y-4 pb-0">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="bg-purple-200 p-2.5 rounded-xl text-purple-700"><Wand2 className="h-5 w-5" /></div>
                    <div className="text-left">
                      <p className="font-bold text-sm text-gray-900 group-hover:text-purple-800">Magic Add (AI)</p>
                      <p className="text-[11px] text-gray-500 mt-0.5">Let AI find the perfect next spot.</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="bg-gray-100 group-hover:bg-blue-100 p-2.5 rounded-xl text-gray-600 group-hover:text-blue-600 transition-colors"><MapPin className="h-5 w-5" /></div>
                    <div className="text-left">
                      <p className="font-bold text-sm text-gray-900 group-hover:text-blue-600">Manual Entry</p>
                      <p className="text-[11px] text-gray-500 mt-0.5">Search or pin a specific place.</p>
                    </div>
                  </div>
                  <ArrowRight className="h-4 w-4 text-gray-300 group-hover:text-blue-400" />
                </button>
                <PopularPlacesSection destinations={destinations} onSelect={handlePlaceSelect} tripData={tripData} category={type} />
              </div>

              {/* Sticky Footer for Choose Step */}
              <div className="sticky bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md pt-3 pb-5 px-5 border-t border-gray-100 shadow-[0_-4px_12px_rgba(0,0,0,0.03)] z-10 transition-all duration-300">
          ...
          <div className="px-5 py-4">
            {step === "ai_loading" && (
              <div className="py-14 flex flex-col items-center justify-center text-center">
                ...
                <h4 className="font-bold text-gray-900 text-base mb-1">Scanning local area…</h4>
...
function DaySelector({
  tripData, targetDay, setTargetDay, accentColor,
}: {
  tripData: DayPlan[]; targetDay: number; setTargetDay: (i: number) => void; accentColor: string;
}) {
  return (
    <div className="mt-2">
      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5 ml-1">Add to Day</label>
      <div className="flex gap-2 flex-wrap">
        {tripData.map((d, i) => (
          <button key={d.day} type="button" onClick={() => setTargetDay(i)}
            className={cn(
              "flex flex-col items-center px-3 py-2 rounded-xl border transition-all",
              targetDay === i ? "text-white shadow-md" : "border-gray-200 bg-gray-50 hover:border-gray-300 hover:bg-gray-100 text-gray-700"
            )}
            style={targetDay === i ? { backgroundColor: accentColor, borderColor: accentColor } : undefined}
          >
            <span className="text-xs font-bold font-primary">Day {d.day}</span>
            <span className={cn("text-[10px] whitespace-nowrap", targetDay === i ? "text-white/70" : "text-gray-400")}>
              {d.date.split(",")[0]}
            </span>
          </button>
        ))}

...
const AIRLINES_BY_REGION: Record<
  string,
...
> = {
  ME: [
    ...
    },
    {
      name: "Mahan Air",
      code: "W5",
      logo: "https://logo.clearbit.com/mahan.aero",
    },
  ],
  ASIA: [
  ...
  OTHER: [
    ...
    { name: "LATAM", code: "LA", logo: "https://logo.clearbit.com/latam.com" },
    {
      ...
      code: "SA",
      logo: "https://logo.clearbit.com/flysaa.com",
    },
  ],
};
...
export async function searchAviationstackFlights(
  ...
  for (let i = 0; i < Math.min(airlines.length * 2, 12); i++) {
    ...
    const stopCities =
      stops > 0
        ? [stopCityPool[Math.floor(Math.random() * stopCityPool.length)]]
        : [];
